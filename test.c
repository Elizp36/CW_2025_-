...
Rgb **draw_Romb(Rgb **arr, BitmapInfoHeader* bmif, Point center, int size, Rgb fill_color);
void print_help();
...
Rgb **draw_Romb(Rgb **arr, BitmapInfoHeader* bmif, Point center, int size, Rgb fill_color) {
    ...
    int down = height/2;
    int left;
    size = (int)sqrt(pow(height,2)+pow(width,2));
...
Rgb **ramka (Rgb **arr, BitmapInfoHeader* bmif, int thickness, Rgb color){
    int old_height = (int)bmif->height;
    int old_width = (int)bmif->width;

    int new_h = old_height + (2*thickness);
    int new_w = old_width + (2*thickness);
    int new_imageSize = new_h*new_w*3+padding(new_w)*new_h;

    bmif->height = new_h;
    bmif->width = new_w;
    bmif->imageSize = new_h * row_len(new_w);
    ...
    for (int y = 0; y < new_h; y++){
        for (int x = 0; x < new_w; x++){
            if(x < thickness || x >= (new_w - thickness) ||
            y < thickness || y >= (new_h - thickness)){
                new_arr[y][x] = color;
            }else{
                int old_x = x - thickness;
                int old_y = y - thickness;
                new_arr[y][x] = arr[old_y][old_x];
            }
        }
    }
    return new_arr;
...
Rgb** binarize_image(Rgb** image, int height, int width, int threshold) {
    // Проверка корректности порога
    if (threshold < 0 || threshold > 765) {
        fprintf(stderr, "Threshold must be between 0 and 765\n");
        return image;
    }
    
    // Применяем бинаризацию к каждому пикселю
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            // Вычисляем сумму компонентов цвета
            int sum = image[y][x].r + image[y][x].g + image[y][x].b;
...
int main(int argc, char *argv[]) {
    ...
    const struct option long_options[] = {
        ...
        {"romb", no_argument, 0, 'O'},
        {"ramka", no_argument, 0, 'M'},
    ...
    int opt;
    while ((opt = getopt_long(argc, argv, "cmCMa:l:r:d:s:n:R:t:L:fF:i:o:hD", long_options, NULL)) != -1) {
        switch (opt) {
            case 'c':
                if (optarg != NULL){
            ...
            case 'O':
                if (optarg != NULL){
                    Error_Handling("Flag --romb does not accept arguments");
                }
                
                if (operation.operation != op_none) {
                    Error_Handling("Only one operation is available at a time");
                }
                operation.operation = op_romb;
                break;
            
            case 'M':
                operation.operation = op_ramka;
                break;
            ...
            case 'n':
                if (operation.operation != op_circle && operation.operation != op_romb) {
                    Error_Handling("This parameter is only available for the --circle or operation");
                }
            ...
            case 'F':
                if (operation.operation != op_circle && operation.operation != op_romb) {
                    Error_Handling("This parameter is only available for the --circle or --romb operation");
                }
    ...
    Rgb **arr = read_bmp(inputFile, &bmfh, &bmif);
     ...
     switch (operation.operation) {
        ...
        case op_circle:
            ...
            break;
        case op_romb:
            write_bmp(outputFile, draw_Romb(arr, &bmif, operation.center, operation.size, operation.fill_color), bmfh, bmif);
            break;
        case op_ramka:
            write_bmp(outputFile, ramka(arr, &bmif, operation.thickness, operation.color), bmfh, bmif);
            break;
        default:
            break;
    }
